...
public class BookController {
    ...
    public ResponseEntity<BookResponse> createBook(@RequestPart BookAddRequest bookAddRequest, @RequestPart("bookImage") MultipartFile bookImage) {
        BookResponse bookResponse = bookService.addBook(bookAddRequest);

        try {
            bookImage.transferTo(new File("/users/kimkiwoong/file/" + bookResponse.getBookId() + ".png"));
        } catch (IOException e) {
